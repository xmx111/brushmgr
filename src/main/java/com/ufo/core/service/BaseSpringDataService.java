...
import com.ufo.core.pagination.PropertyFilter.MatchType;
...
@Transactional
public abstract class BaseSpringDataService<T extends Persistable<? extends Serializable>, ID extends Serializable> {

    private static final String sess_oper = "session_operation";
    
    protected final Logger logger = LoggerFactory.getLogger(BaseSpringDataService.class);
    ...
    public T save(T entity) {
        if (entity.isNew()) {
            if (entity instanceof OperatorEntity){
                ((OperatorEntity) entity).setCreateOperator(this.getOperation());
     ...
     *            待操作数据
     */
    public void delete(T entity, boolean isClear) {
        if (entity instanceof OperatorEntity && !isClear){
            OperatorEntity obj = (OperatorEntity)this.findOne((ID)entity.getId());
            ((OperatorEntity) entity).setCreateOperator(obj.getCreateOperator());
            ((OperatorEntity) entity).setCreateTime(obj.getCreateTime());
            ((OperatorEntity) entity).setUpdateOperator(obj.getUpdateOperator());
            ((OperatorEntity) entity).setUpdateTime(obj.getUpdateTime());
            ((OperatorEntity) entity).setDeleted(UndeleteEntity.DeleteTypeEnum.DELETE);
            ((OperatorEntity) entity).setDeleteOperator(this.getOperation());
            ((OperatorEntity) entity).setDeleteTime(this.getCurrentTime());
    ...
    */
     ...
     * @param builder
     */
    public List<Predicate> appendPredicate(Root<T> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
        return null;
    }
    ...
    protected Object setCreateOper(Object t){
        if (t instanceof OperatorEntity){
            ((OperatorEntity) t).setCreateOperator(this.getOperation());
            ((OperatorEntity) t).setCreateTime(this.getCurrentTime());
        }
        return t;
    }

    protected Object setUpdateOper(Object t){
        if (t instanceof OperatorEntity){
            ((OperatorEntity) t).setUpdateOperator(this.getOperation());
            ((OperatorEntity) t).setUpdateTime(this.getCurrentTime());
        }
        return t;
    }
